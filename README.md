...
| [XISOSharp.Tests](XISOSharp.Tests/) | xUnit suite (1098 tests) — snapshot `test_fixture.iso` + corruption resilience + `MemoryBlockDevice` + `xdvdfs-cli` split-CSO interop + extract-xiso 2.7.1 legacy-layout interop + unpack-resume/output-guard/`-d`-edge-case/stream-API/filesystem-destination/robustness coverage |
...
dotnet test -c Release                       # 1098 tests (`XISOSharp.Tests`; ZARSharp lives in `../CSharp_ZARSharp`)
```
