...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        foreach (var dir in dirs)
        {
            ...
            if (args.Contains("--recursive"))
            {
                ...
                // dedup already added
                foreach (var f in rec) if (!isoFiles.Contains(f, StringComparer.OrdinalIgnoreCase)) isoFiles.Add(f);
        ...
        if (isoFiles.Count == 0)
        {
            var fallback = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "TestData", "output", "source.iso");
            ...
            else
            {
                ...
                else
                {
                    ...
                    var tmpDir = Path.Combine(Path.GetTempPath(), "battle_synth_src_" + Guid.NewGuid().ToString("N")[..8]);
                    ...
                    try
                    {
                        var q = XISOSharp.Logger.Quiet; XISOSharp.Logger.Quiet = true;
    ...
    private static string[] ParseDirs(string[] args)
    {
        ...
        if (list.Count == 0)
        {
            ...
            foreach (var d in defaults) if (Directory.Exists(d)) list.Add(d);
    ...
    private static string[] ParseCreateDirs(string[] args)
    {
        ...
        for (int i = 0; i < args.Length - 1; i++)
            if (args[i] == "--create-dirs" || args[i] == "--create")
                list.AddRange(args[i + 1].Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries));
        // Also supports --create-dirs=<a,b>
        foreach (var a in args)
            if (a.StartsWith("--create-dirs=", StringComparison.Ordinal))
                list.AddRange(a.Substring("--create-dirs=".Length).Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries));
        ...
        if (list.Count == 0)
        {
            ...
            {
                var alt = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "TestData", "source"));
    ...
    private static string[] ParseIsoArgs(string[] args)
    {
        ...
        var skipNext = false;
        var optionValues = new HashSet<string>(StringComparer.Ordinal) { "--exe", "--native", "--dirs", "--create-dirs", "--create", "--limit" };
        for (int i = 0; i < args.Length; i++)
        {
            if (skipNext) { skipNext = false; continue; }
            var a = args[i];
            if (optionValues.Contains(a)) { skipNext = true; continue; }
            if (a.StartsWith("--dirs=", StringComparison.Ordinal) || a.StartsWith("--create-dirs=", StringComparison.Ordinal) || a.StartsWith("--limit=", StringComparison.Ordinal)) continue;
    ...
    private static int ParseLimit(string[] args)
    {
        ...
        for (int i = 0; i < args.Length - 1; i++)
            if (args[i] == "--limit" && int.TryParse(args[i + 1], out var n)) return n;
        foreach (var a in args)
            if (a.StartsWith("--limit=", StringComparison.Ordinal) && int.TryParse(a.Substring("--limit=".Length), out var n)) return n;
...
");
    ...
    private static void PrintSummary(BattleSessionResult s)
    {
        ...
        Console.WriteLine($"  Files: {s.TotalFiles} total | {s.PassedFiles} passed | {s.FailedFiles} failed | {s.SkippedFiles} skipped");
        Console.WriteLine($"  Checks: {s.TotalSubTests} total | {s.PassedSubTests} passed | {s.FailedSubTests} failed | {s.SkippedSubTests} skipped");
        var color = s.FailedSubTests > 0 ? ConsoleColor.Red : ConsoleColor.Green;
        var prev = Console.ForegroundColor;
        Console.ForegroundColor = color;
        Console.WriteLine(s.FailedSubTests == 0 ? "  RESULT: ALL CHECKS PASSED ✓" : $"  RESULT: {s.FailedSubTests} CHECK(S) FAILED ✗");
    ...
    private static void WriteReports(BattleSessionResult s, IList<string> isoFiles, string exePath)
    {
        try
        {
            ...
            using (var w = new StreamWriter(txtPath))
            {
                ...
                w.WriteLine($"Summary: {s.PassedSubTests}/{s.TotalSubTests} checks passed, {s.FailedSubTests} failed in {s.Elapsed.TotalSeconds:F1}s");
                w.WriteLine();
                foreach (var f in s.FileResults)
                {
                    w.WriteLine($"{f.FileName} ({f.FileSize} bytes) - {(f.HasFailures ? "FAIL" : "PASS")} {f.ElapsedSeconds:F1}s");
            ...
            var json = System.Text.Json.JsonSerializer.Serialize(new
            {
                ...
                files = s.FileResults.Select(f => new
                {
                    ...
                    subTests = f.SubTests.Select(st => new { st.TestName, status = st.Status.ToString(), st.Detail, st.ElapsedSeconds })
                })
