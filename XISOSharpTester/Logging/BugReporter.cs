...
internal static class BugReporter
{
    ...
    private static async Task SendAsync(string fullMessage, string stackTrace)
    {
        ...
        var payload = new
        {
            message = Truncate(fullMessage, MaxMessage),
            applicationName = Truncate(ApplicationName, MaxAppName),
            version = Truncate(version, MaxVersion),
            userInfo = (string?)null,
            environment = Truncate(environment, MaxEnvironment),
