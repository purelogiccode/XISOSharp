...
public class XisoOperationsTests : IDisposable
{
    ...
    {
        string dir = Path.Combine(Path.GetTempPath(), $"xiso_ops_{Guid.NewGuid():N}");
    ...
    {
        string src = Path.Combine(Path.GetTempPath(), $"xiso_ops_src_{Guid.NewGuid():N}");
    ...
    public void ExtractFiller_CreatesFillerFile_QuietTrue()
    {
        ...
        string outDir = CreateTempDir();
        string fillerPath = Path.Combine(outDir, "out.filler");

        bool ok = XisoOperations.ExtractFiller(iso, fillerPath, 0, null, quiet: true);
    ...
    public void ExtractFiller_CreatesFillerFile_QuietFalse()
    {
        ...
        bool ok = XisoOperations.ExtractFiller(iso, fillerPath, 0, null, quiet: false);
    ...
    public void ExtractFiller_FileStreamOverload_Succeeds()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string fillerPath = Path.Combine(outDir, "stream.filler");

        using FileStream fs = new(iso, FileMode.Open, FileAccess.Read, FileShare.Read, 65536);
        bool ok = XisoOperations.ExtractFiller(fs, 0, fs.Length, fillerPath, quiet: true);
    ...
    public void ExtractFiller_WithXisoLengthOverride_Succeeds()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string fillerPath = Path.Combine(outDir, "override.filler");
        long isoLen = new FileInfo(iso).Length;

        bool ok = XisoOperations.ExtractFiller(iso, fillerPath, 0, xisoLengthOverride: isoLen, quiet: true);
    ...
    public void ExtractFiller_WithIsoOffset_PrependedIso_Succeeds()
    {
        ...
        string outDir = CreateTempDir();
        string fillerPath = Path.Combine(outDir, "prepend.filler");
        const long offset = 16L * Constants.SectorSize;

        bool ok = XisoOperations.ExtractFiller(iso, fillerPath, offset, null, quiet: true);
    ...
    public void ExtractFiller_FillerSizeMatchesExpected()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string fillerPath = Path.Combine(outDir, "size.filler");
        ...
        List<(uint Start, uint End)> merged = XisoRanges.MergeRanges(sys, files);
        long validBytes = 0;
    ...
    {
        string outDir = CreateTempDir();
        string missing = Path.Combine(outDir, "nonexistent.iso");
        string filler = Path.Combine(outDir, "out.filler");
    ...
    public void ExtractFiller_Cancellation_ThrowsOperationCanceledException()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string filler = Path.Combine(outDir, "cancel.filler");
    ...
    public void ExtractSeed_NonXgd1_ReturnsNull()
    {
        ...
        uint? seed = XisoOperations.ExtractSeed(iso, 0, quiet: true);
    ...
    public void TryExtractSeed_NonXgd1_ReturnsFalseAndDoesNotCreateFile()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string seedPath = Path.Combine(outDir, "seed.bin");
        if (File.Exists(seedPath)) File.Delete(seedPath);

        bool ok = XisoOperations.TryExtractSeed(iso, seedPath, 0, quiet: true);
    ...
    public void ExtractSeed_WithIsoOffset_PrependedIso_ReturnsNullForNonXgd1()
    {
        ...
        uint? seed = XisoOperations.ExtractSeed(iso, offset, quiet: true);

        Assert.Null(seed);
        string outDir = CreateTempDir();
        string seedPath = Path.Combine(outDir, "seed2.bin");
        bool ok = XisoOperations.TryExtractSeed(iso, seedPath, offset, quiet: true);
        Assert.False(ok);
    }
    ...
    public void WipeFiller_CreatesOutputSameSizeAndExtractable()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string wiped = Path.Combine(outDir, "wiped.iso");

        bool ok = XisoOperations.WipeFiller(iso, wiped, 0, quiet: true);
        ...
        int res = XisoReader.Extract(wiped, extractDir, false);
        Assert.Equal(0, res);
    ...
    public void TrimXiso_WithOutput_ReducesOrEqualsSizeAndRemainsValid()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string trimmed = Path.Combine(outDir, "trimmed.iso");
        long origLen = new FileInfo(iso).Length;

        bool ok = XisoOperations.TrimXiso(iso, trimmed, 0, quiet: true);
        ...
        string extractDir = CreateTempDir();
        int res = XisoReader.Extract(trimmed, extractDir, false);
        Assert.Equal(0, res);
        Assert.True(File.Exists(Path.Combine(extractDir, "a.txt")));
    }
    ...
    public void TrimXiso_InPlace_TrimsFileAndRemainsValid()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string inPlace = Path.Combine(outDir, "inplace.iso");
        File.Copy(iso, inPlace, true);
        long before = new FileInfo(inPlace).Length;

        bool ok = XisoOperations.TrimXiso(inPlace, null, 0, quiet: true);
        ...
        string extractDir = CreateTempDir();
        int res = XisoReader.Extract(inPlace, extractDir, false);
        Assert.Equal(0, res);
    ...
    public void WipeAndTrim_CreatesSmallerOutputAndRemainsValid()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string wipeTrimmed = Path.Combine(outDir, "wipetrim.iso");
        long origLen = new FileInfo(iso).Length;

        bool ok = XisoOperations.WipeAndTrim(iso, wipeTrimmed, 0, quiet: true);
        ...
        // Verify extractable
        string extractDir = CreateTempDir();
        int res = XisoReader.Extract(wipeTrimmed, extractDir, false);
        Assert.Equal(0, res);
        Assert.True(File.Exists(Path.Combine(extractDir, "a.txt")));
        Assert.Equal("hello", File.ReadAllText(Path.Combine(extractDir, "a.txt")));
    }
    ...
    public void WipeAndTrim_MissingFile_ThrowsFileNotFoundException()
    {
        string outDir = CreateTempDir();
        string missing = Path.Combine(outDir, "nonexistent.iso");
        string outPath = Path.Combine(outDir, "wipetrim.iso");
    ...
    public void ExtractFiller_AndWipeFiller_FillerAndWipedAreConsistent()
    {
        ...
        string iso = CreateIso(src);
        string outDir = CreateTempDir();
        string filler = Path.Combine(outDir, "filler.bin");
        string wiped = Path.Combine(outDir, "wiped.iso");
