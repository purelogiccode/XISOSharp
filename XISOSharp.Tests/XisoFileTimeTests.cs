...
public class XisoFileTimeTests : IDisposable
{
    ...
    {
        string dir = Path.Combine(Path.GetTempPath(), $"xiso_ft_{Guid.NewGuid():N}");
    ...
    {
        DateTimeOffset dto = new(2020, 6, 15, 12, 34, 56, TimeSpan.Zero);
    ...
    public void FileTimeHelper_Max_Handling()
    {
        ...
        Assert.True(FileTimeHelper.TryParseFileTime("0", out ulong r0, out DateTimeOffset d0));
        Assert.Equal(0UL, r0);
        Assert.Equal(new DateTimeOffset(1601, 1, 1, 0, 0, 0, TimeSpan.Zero), d0);

        Assert.True(FileTimeHelper.TryParseFileTime("now", out ulong rNow, out DateTimeOffset dNow));
        Assert.True(rNow > 0);
        ...
        Assert.Equal(0UL, rh);
        Assert.True(FileTimeHelper.TryParseFileTime("2023-08-26T15:30:00Z", out ulong rIso, out DateTimeOffset dIso));
    ...
    public void XisoReader_GetSetFileTime_RoundTrip()
    {
        ...
        DateTimeOffset targetDto = new(2021, 12, 31, 23, 59, 59, TimeSpan.Zero);
        ...
        FileBlockDevice dev = new(fs, false);
    ...
    public void XisoReader_FileTime_RewritePreserved()
    {
        ...
        string isoPath = CreateSimpleIso(out string outDir);
        DateTimeOffset knownDto = new(2019, 5, 17, 10, 0, 0, TimeSpan.Zero);
