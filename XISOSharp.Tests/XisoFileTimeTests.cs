...
public class XisoFileTimeTests : IDisposable
{
    ...
    public void FileTimeHelper_RoundTrip_DtoToRawAndBack()
    {
        ...
        var raw2 = FileTimeHelper.ToFileTimeRaw(back);
    ...
    public void FileTimeHelper_Max_Handling()
    {
        ...
        Assert.True(FileTimeHelper.TryParseFileTime("0", out var r0, out var d0));
        Assert.Equal(0UL, r0);
        ...
        Assert.Equal(0UL, rh);
        Assert.True(FileTimeHelper.TryParseFileTime("2023-08-26T15:30:00Z", out var rIso, out var dIso));
        Assert.Equal(new DateTimeOffset(2023, 8, 26, 15, 30, 0, TimeSpan.Zero), dIso);
        Assert.True(rIso > 0);

        // Decimal raw parsing
        var dec = maxRaw.ToString();
        Assert.True(FileTimeHelper.TryParseFileTime(dec, out var rDec, out _));
    ...
    {
        var isoPath = CreateSimpleIso(out _);
        ...
        var zeroRaw = XisoReader.GetFileTimeRaw(isoPath);
        ...
        const ulong hexRaw = 0x01D7A3C8F1234567UL; // arbitrary valid
        XisoReader.SetFileTime(isoPath, hexRaw);
        var readBack = XisoReader.GetFileTimeRaw(isoPath);
        ...
        var devRaw = XisoReader.GetFileTimeRaw(dev, "test.iso");
    ...
    public void XisoReader_FileTime_RewritePreserved()
    {
        ...
        var oldPath = isoPath + ".old";
        File.Move(isoPath, oldPath);
        try
        {
            var rc = XisoReader.Rewrite(oldPath, outDir, out var rewriteOut, CancellationToken.None, null, null,
                null,
                null);
            Assert.Equal(0, rc);
            ...
            var rewrittenIso = rewriteOut ?? Directory.GetFiles(outDir, "*.iso").FirstOrDefault() ?? isoPath;
