...
public class CisoTests : IDisposable
{
    ...
    public void CompressToCso_FromIsoFile_ProducesCsoAndIsCsoTrue()
    {
        ...
        string csoPath = Path.Combine(csoDir, "out.cso");
    ...
    {
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "dir.cso");
    ...
    {
        string isoPath = CreateTempIso();
        byte[] origHash = ComputeSha256(isoPath);

        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "rt.cso");
        int cRc = CisoWriter.CompressToCso(isoPath, csoPath, level: 6);
        Assert.Equal(0, cRc);

        string decDir = CreateTempDir();
        string decPath = Path.Combine(decDir, "rt.iso");
        int dRc = CisoReader.DecompressToIso(csoPath, decPath);
        Assert.Equal(0, dRc);
        Assert.True(File.Exists(decPath));

        byte[] decHash = ComputeSha256(decPath);
    ...
    public void CompressStream_DecompressStream_MemoryStreams_RoundTrip()
    {
        ...
        using MemoryStream src = new(isoBytes, writable: false);
    ...
    public void CompressToCso_WithLevelZero_StoresPlainAndRoundTrips()
    {
        ...
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "level0.cso");
        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 0);
        Assert.Equal(0, rc);
        Assert.True(CisoReader.IsCso(csoPath));

        string decDir = CreateTempDir();
        string decPath = Path.Combine(decDir, "level0.iso");
        int dRc = CisoReader.DecompressToIso(csoPath, decPath);
        Assert.Equal(0, dRc);
    ...
    public void CompressToCso_WithVariousLevels_RoundTrip()
    {
        ...
        {
            string csoDir = CreateTempDir();
            string csoPath = Path.Combine(csoDir, $"level{level}.cso");
            int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: level);
            Assert.Equal(0, rc);
            Assert.True(CisoReader.IsCso(csoPath));

            string decDir = CreateTempDir();
            string decPath = Path.Combine(decDir, $"level{level}.iso");
            int dRc = CisoReader.DecompressToIso(csoPath, decPath);
            Assert.Equal(0, dRc);
            Assert.Equal(origHash, ComputeSha256(decPath));
        }
    ...
    public async Task CompressToCsoAsync_And_DecompressToIsoAsync_RoundTrip()
    {
        ...
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "async.cso");
        ...
        string decDir = CreateTempDir();
        string decPath = Path.Combine(decDir, "async.iso");
    ...
    public void CompressStream_And_DecompressStream_ViaFileStreams_RoundTrip()
    {
        ...
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "stream.cso");
        using (FileStream src = new(isoPath, FileMode.Open, FileAccess.Read, FileShare.Read, 65536))
        using (FileStream dst = new(csoPath, FileMode.Create, FileAccess.Write, FileShare.None, 65536))
        {
        ...
        string decDir = CreateTempDir();
        string decPath = Path.Combine(decDir, "stream.iso");
        using (FileStream src = new(csoPath, FileMode.Open, FileAccess.Read, FileShare.Read, 65536))
        using (FileStream dst = new(decPath, FileMode.Create, FileAccess.Write, FileShare.None, 65536))
        {
    ...
    public void ReadFromCso_StringOverload_ReadsHeaderAtZero()
    {
        ...
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "read.cso");
        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 6);
        Assert.Equal(0, rc);
    ...
    public void ReadFromCso_FileStreamOverload_ReadsSectorZero()
    {
        ...
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "read2.cso");
        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 6);
        Assert.Equal(0, rc);
    ...
    public void ReadFromCso_RandomAccess_CrossSectorAndMidFile()
    {
        ...
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "read3.cso");
        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 6);
        Assert.Equal(0, rc);
    ...
    public void IsCso_ReturnsFalse_ForTruncatedFile()
    {
        ...
        string truncated = Path.Combine(tmpDir, "trunc.cso");
    ...
    public void CompressToCso_MissingSource_ThrowsFileNotFoundException()
    {
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "out.cso");
        string missing = Path.Combine(csoDir, "does_not_exist.iso");
    ...
    {
        string tmpDir = CreateTempDir();
        string missing = Path.Combine(tmpDir, "missing.cso");
        string outIso = Path.Combine(tmpDir, "out.iso");
    ...
    public void DecompressStream_TruncatedCso_ThrowsInvalidDataException()
    {
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "trunc2.cso");
        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 6);
        Assert.Equal(0, rc);

        byte[] bytes = File.ReadAllBytes(csoPath);
        // Truncate to header + partial index (corrupt)
        byte[] truncated = bytes[..((int)CisoWriter.HeaderSize + 4)]; // header + one index entry, not full

        using MemoryStream src = new(truncated, writable: false);
    ...
    public void DecompressStream_TruncatedPayload_ThrowsInsteadOfZeroPadding()
    {
        ...
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "truncpay.cso");
        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 6);
        Assert.Equal(0, rc);

        byte[] bytes = File.ReadAllBytes(csoPath);
        ulong uncompressedSize = BinaryPrimitives.ReadUInt64LittleEndian(bytes.AsSpan(8, 8));
        uint blockSize = BinaryPrimitives.ReadUInt32LittleEndian(bytes.AsSpan(16, 4));
        ulong totalBlocks = (uncompressedSize + blockSize - 1) / blockSize;
        long payloadStart = 24 + (((long)totalBlocks + 1) * 4);
        Assert.True(bytes.Length > payloadStart + 2048, "test CSO too small to truncate mid-payload");

        byte[] cut = bytes[..(int)(payloadStart + 100)];
        using MemoryStream src = new(cut, writable: false);
        using MemoryStream dst = new();
    ...
    public void ReadFromCso_OutOfRange_ThrowsArgumentOutOfRangeException()
    {
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "oor.cso");
        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 6);
        Assert.Equal(0, rc);
    ...
    public void DecompressStream_InvalidIndexCorruption_ThrowsInvalidDataException()
    {
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "corrupt.cso");
        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 6);
        Assert.Equal(0, rc);
        ...
        if (bytes.Length > 32)
        {
            ...
            if (indexLen >= 2 && bytes.Length >= 24 + 8)
            {
                ...
                uint first = BinaryPrimitives.ReadUInt32LittleEndian(bytes.AsSpan(24, 4));
                uint second = first - 1;
                BinaryPrimitives.WriteUInt32LittleEndian(bytes.AsSpan(28, 4), second);
            }
        }

        string corruptPath = Path.Combine(csoDir, "corrupt2.cso");
        File.WriteAllBytes(corruptPath, bytes);

        using FileStream src = new(corruptPath, FileMode.Open, FileAccess.Read, FileShare.Read, 65536);
        using MemoryStream dst = new();
        Assert.Throws<InvalidDataException>(() => CisoReader.DecompressStream(src, dst));
    }
    ...
    {
        byte[] dst = new byte[Lz4.MaxCompressedOutputSize(input.Length)];
        int n = Lz4.Compress(input, dst);
        Assert.True(n > 0);
    ...
    public void Lz4_Compress_Decompress_RoundTrips_RandomAndStructuredData()
    {
        ...
        for (int t = 0; t < 100; t++)
        {
            ...
            while (pos < len)
            {
                int run = Math.Min(rng.Next(1, 40), len - pos);
    ...
    {
        byte[] dst = new byte[Lz4.MaxCompressedOutputSize(data.Length)];
        int n = Lz4.Compress(data, dst);
        byte[] back = new byte[data.Length];
        int m = Lz4.Decompress(dst.AsSpan(0, n), back);
    ...
    private static (byte version, byte align, uint[] index) ParseCsoHeader(string path)
    {
        ...
        long totalBlocks = (long)((uncompressedSize + CisoWriter.BlockSize - 1) / CisoWriter.BlockSize);
        uint[] index = new uint[totalBlocks + 1];
    ...
    public void CompressToCso_DefaultVersion_WritesV2HeaderWithAlign2()
    {
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "v2.cso");

        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 9);
        Assert.Equal(0, rc);
        Assert.True(CisoReader.IsCso(csoPath));

        (byte version, byte align, _) = ParseCsoHeader(csoPath);
    ...
    public void CompressToCso_Version2_RoundTrip_PreservesSha256()
    {
        ...
        foreach (int level in new[] { 0, 1, 9 })
        {
            string csoDir = CreateTempDir();
            string csoPath = Path.Combine(csoDir, $"v2l{level}.cso");
            int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: level, version: CisoWriter.VersionLz4);
            Assert.Equal(0, rc);
            Assert.True(CisoReader.IsCso(csoPath));

            string decDir = CreateTempDir();
            string decPath = Path.Combine(decDir, $"v2l{level}.iso");
    ...
    public void CompressToCso_Version2_Level0_AllSectorsPlain()
    {
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "v2store.cso");

        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 0, version: CisoWriter.VersionLz4);
        Assert.Equal(0, rc);

        (_, _, uint[] index) = ParseCsoHeader(csoPath);
    ...
    public void CompressToCso_Version2_Level9_CompressesSectors()
    {
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "v2best.cso");

        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: 9, version: CisoWriter.VersionLz4);
        Assert.Equal(0, rc);

        (_, _, uint[] index) = ParseCsoHeader(csoPath);
        // The XISO has large 0xFF gap regions; at least one sector must store compressed.
        int compressed = index.Take(index.Length - 1).Count(e => (e & 0x80000000u) != 0);
    ...
    public void CompressToCso_InvalidVersion_ThrowsArgumentOutOfRange()
    {
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "bad.cso");
        ...
        long splitBytes = 16384, int level = 9, byte version = CisoWriter.VersionLz4)
    {
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, outputName ?? "split.cso");
        int rc = CisoWriter.CompressToCso(isoPath, csoPath, level: level, splitBytes: splitBytes, version: version);
        Assert.Equal(0, rc);
    ...
    private static List<string> SplitParts(string csoPath)
    {
        ...
        string baseName = csoPath[..^4]; // strip ".cso"
        for (int i = 1;; i++)
        {
            string part = $"{baseName}.{i}.cso";
    ...
    public void CompressToCso_WithSplit_ExistingPart_ThrowsIOException()
    {
        ...
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "clash.cso");
    ...
    {
        (string isoPath, string csoPath, _) = CreateSplitCso(splitBytes: 16384);
        ...
        byte[] origHash = ComputeSha256(isoPath);
        string decDir = CreateTempDir();
        string decPath = Path.Combine(decDir, "split.iso");
    ...
    {
        (_, string csoPath, _) = CreateSplitCso(splitBytes: 1 << 30);
        List<string> parts = SplitParts(csoPath);
        string singlePart = Assert.Single(parts);
    ...
    {
        (_, _, string csoDir) = CreateSplitCso(outputName: "noext", splitBytes: 16384);
    ...
    {
        (string isoPath, string csoPath, _) = CreateSplitCso(splitBytes: 8192);
        byte[] isoBytes = File.ReadAllBytes(isoPath);
        List<string> parts = SplitParts(csoPath);
        ...
        long boundary = new FileInfo(parts[0]).Length;
        if (boundary + 512 < isoBytes.Length)
        {
            byte[] cross = new byte[1024];
    ...
    public void CisoBlockDevice_SplitInput_MatchesOriginalIso()
    {
        ...
        using CisoBlockDevice dev = new(parts[0]);
        Assert.Equal(isoBytes.Length, dev.Length);

        byte[] buf = new byte[4096];
        for (int offset = 0; offset < isoBytes.Length; offset += buf.Length)
        {
            int n = dev.Read(offset, buf);
            int expected = Math.Min(buf.Length, isoBytes.Length - offset);
    ...
    public void CompressToCso_SplitVersion1_RoundTrips()
    {
        ...
        (byte version, _, _) = ParseCsoHeader(parts[0]);
        Assert.Equal(CisoWriter.VersionDeflate, version);

        byte[] origHash = ComputeSha256(isoPath);
        string decDir = CreateTempDir();
        string decPath = Path.Combine(decDir, "splitv1.iso");
        Assert.Equal(0, CisoReader.DecompressToIso(parts[0], decPath));
        Assert.Equal(origHash, ComputeSha256(decPath));
    }
    ...
    public void DecompressStream_TerabyteClaimInTinyFile_ThrowsBeforeAllocating()
    {
        ...
        string path = Path.Combine(dir, "liar.cso");
    ...
    public void GetVolumeInfo_CsoPath_SeesDecompressedView()
    {
        ...
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "vol.cso");
        Assert.Equal(0, CisoWriter.CompressToCso(isoPath, csoPath, level: 6));

        VolumeInfo vol = XisoReader.GetVolumeInfo(csoPath);
    ...
    public void GetSectorLayout_CsoPath_RoutesThroughDecompressedView()
    {
        ...
        string isoPath = CreateTempIso();
        string csoDir = CreateTempDir();
        string csoPath = Path.Combine(csoDir, "layout.cso");
        Assert.Equal(0, CisoWriter.CompressToCso(isoPath, csoPath, level: 6));

        SectorLayout layout = XisoReader.GetSectorLayout(csoPath);
