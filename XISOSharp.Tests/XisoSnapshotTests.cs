...
public class XisoSnapshotTests : IDisposable
{
    ...
    internal static void BuildSnapshotTree(string root)
    {
        ...
        for (int i = 0; i < binary.Length; i++) binary[i] = (byte)(i % 256);
        ...
        string nested = Path.Combine(root, "nested", "level1", "level2");
        ...
        string sysUpdate = Path.Combine(root, "$SystemUpdate");
    ...
    internal static byte[] ExpectedPatchedXbe(byte[] source)
    {
        ...
        for (int i = 0; i + pattern.Length <= patched.Length; i++)
        {
    ...
    {
        int rc = XisoWriter.CreateXiso(srcDir, isoDir, null, null, out string? isoPath,
    ...
    public void Snapshot_CreateTwiceWithFileTimeZero_ByteIdentical()
    {
        ...
        string first = CreateSnapshotIso(src, CreateTempDir("xiso_snap_a"));
        string second = CreateSnapshotIso(src, CreateTempDir("xiso_snap_b"));
    ...
    public void Snapshot_MatchesCheckedInFixture()
    {
        ...
        string src = CreateTempDir("xiso_snap_src");
        BuildSnapshotTree(src);
        string created = CreateSnapshotIso(src, CreateTempDir("xiso_snap_out"));

        byte[] expected = File.ReadAllBytes(FixtureIsoPath);
        byte[] actual = File.ReadAllBytes(created);
    ...
    public void Snapshot_FixtureExtract_AllFileHashesMatchSource()
    {
        ...
        string extractDir = CreateTempDir("xiso_snap_ext");
        ...
        foreach ((string rel, byte[] hash) in expected)
    ...
    public void Snapshot_RewriteThenExtract_PreservesAllContent()
    {
        ...
        string rewriteDir = CreateTempDir("xiso_snap_rw");
        Assert.Equal(0, XisoReader.Rewrite(FixtureIsoPath, rewriteDir, out string? rewritten));
        ...
        string extractDir = CreateTempDir("xiso_snap_rwext");
    ...
    public void RegenerateFixtureIso_WhenRequested()
    {
        ...
        string tempCopy = Path.Combine(CreateTempDir("xiso_snap_regen"), "test_fixture.iso");
        File.Copy(created, tempCopy);

        byte[] expected = File.ReadAllBytes(FixtureIsoPath);
        byte[] actual = File.ReadAllBytes(tempCopy);
        Assert.Equal(expected, actual);
    }
}
