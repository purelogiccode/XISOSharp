...
public class XisoSplitTests : IDisposable
{
    ...
    {
        string outDir = CreateTempDir("xiso_split_out");
    ...
    {
        string src = CreateTempDir("xiso_split_src");
    ...
    {
        string iso = CreateSplitIso();
        string work = CreateTempDir("xiso_split_work");
        ...
        IReadOnlyList<string> parts = XisoReader.SplitXiso(iso, Path.Combine(work, "game"), PartSize);
        Assert.True(parts.Count >= 3);
        Assert.Equal(Path.Combine(work, "game.1.iso"), parts[0]);

        string joined = Path.Combine(work, "rejoined.iso");
    ...
    public void SplitHalves_TwoParts_JoinIdentical()
    {
        ...
        IReadOnlyList<string> parts = XisoSplitter.SplitHalves(iso, Path.Combine(work, "half"));
        Assert.Equal(2, parts.Count);
        Assert.Equal(Path.Combine(work, "half.1.iso"), parts[0]);
        Assert.Equal(Path.Combine(work, "half.2.iso"), parts[1]);

        long first = new FileInfo(parts[0]).Length;
        long second = new FileInfo(parts[1]).Length;
    ...
    public void Split_Parts_SectorAligned_ExceptLast()
    {
        ...
        IReadOnlyList<string> parts = XisoSplitter.Split(iso, Path.Combine(work, "game"), PartSize);
        Assert.True(parts.Count >= 2);
        for (int i = 0; i < parts.Count - 1; i++)
            Assert.Equal(PartSize, new FileInfo(parts[i]).Length);
        long last = new FileInfo(parts[^1]).Length;
    ...
    public void Split_FitsInOnePart_SinglePart()
    {
        ...
        IReadOnlyList<string> parts = XisoSplitter.Split(iso, Path.Combine(work, "game"), long.MaxValue);
        Assert.Single(parts);
        Assert.Equal(Path.Combine(work, "game.1.iso"), parts[0]);
    ...
    public void PartPath_Naming_And_IsSplitPath()
    {
        ...
        string withExt = Path.Combine("d", "game.iso");
    ...
    public void Split_Guards()
    {
        string iso = CreateSplitIso();
        string work = CreateTempDir("xiso_split_work");
        string okBase = Path.Combine(work, "game");
        ...
        byte[] garbageBytes = new byte[100_000];
    ...
    public void Split_RefusesExistingParts_BeforeWriting()
    {
        string iso = CreateSplitIso();
        string work = CreateTempDir("xiso_split_work");
        string basePath = Path.Combine(work, "game");

        IReadOnlyList<string> parts = XisoSplitter.Split(iso, basePath, PartSize);
        byte[] firstBytes = File.ReadAllBytes(parts[0]);
    ...
    public void Join_Guards()
    {
        ...
        string existing = Path.Combine(work, "existing.iso");
    ...
    {
        string work = CreateTempDir("xiso_split_work");
        string first = Path.Combine(work, "garbage.1.iso");
        File.WriteAllBytes(first, new byte[100_000]);
        string output = Path.Combine(work, "joined.iso");
    ...
    public void Join_ByteExactConcat_OrderProof()
    {
        ...
        byte[] part2Bytes = File.ReadAllBytes(parts[1]);
        part2Bytes[100] ^= 0xFF;
        File.WriteAllBytes(parts[1], part2Bytes);

        string joined = Path.Combine(work, "joined.iso");
        byte[] expected = parts.SelectMany(File.ReadAllBytes).ToArray();
    ...
    public void Split_Progress_Monotonic_FullCoverage()
    {
        ...
        string work = CreateTempDir("xiso_split_work");
        long length = new FileInfo(iso).Length;
        List<ProgressInfo> events = new();

        ProgressRecorder progress = new(events);
        ...
        long prev = 0;
        foreach (ProgressInfo e in events)
        {
    ...
    public void Cli_Split_Halves_And_Defaults()
    {
        string iso = CreateSplitIso();
        string work = CreateTempDir("xiso_split_work");
        string staged = Path.Combine(work, "staged.iso");
        ...
        string first = Path.Combine(work, "staged.1.iso");
