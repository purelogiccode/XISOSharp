...
using ExtractXisoTester.Models;
using ExtractXisoTester.Services;
using Microsoft.Win32;
using Serilog;

namespace ExtractXisoTester.ViewModels;
...
internal class MainViewModel : INotifyPropertyChanged
{
    ...
    {
        BrowseExtractXisoCommand = new RelayCommand(_ => BrowseExtractXiso());
    ...
    private void AutoDetectExtractXiso()
    {
        ...
        {
            ExtractXisoPath = candidate;
        }
    }

    private string _extractXisoPath = string.Empty;
    public string ExtractXisoPath
    {
        get => _extractXisoPath;
        set { _extractXisoPath = value;
            OnPropertyChanged();
            OnPropertyChanged(nameof(IsExtractXisoValid));
            OnPropertyChanged(nameof(CanRunTests)); }
    }

    public bool IsExtractXisoValid => !string.IsNullOrEmpty(ExtractXisoPath) && File.Exists(ExtractXisoPath);
    ...
    public ICommand BrowseExtractXisoCommand { get; }
    ...
    private void BrowseExtractXiso()
    {
        ...
        {
            ExtractXisoPath = dlg.FileName;
            AddLog($"extract-xiso.exe set to: {ExtractXisoPath}");
        }
    }
    ...
    private async Task RunTestsAsync()
    {
        ...
        var exePath = IsExtractXisoValid ? ExtractXisoPath : string.Empty;
        if (!IsExtractXisoValid)
        {
    ...
    private void ExportPdf()
    {
        ...
        var dlg = new SaveFileDialog
        {
            ...
            FileName = $"ExtractXisoTester_Results_{DateTime.Now:yyyyMMdd_HHmmss}.pdf"
        };
        if (dlg.ShowDialog() == true)
        {
            try
            {
                PdfExporter.Export(SessionResult, _runner.ExtractXisoVersion, dlg.FileName);
    ...
    private void CopyResults()
    {
        ...
        sb.AppendLine("=== ExtractXiso Tester Results ===");
