...
{
    private static string? _extractXisoVersion;

    public string? ExtractXisoVersion => _extractXisoVersion;

    public async Task<TestSessionResult> RunAsync(
        List<XisoFileEntry> files,
        string extractXisoExePath,
        IProgress<TestProgress>? progress = null)
    {
        var session = new TestSessionResult();
        var exeAvailable = File.Exists(extractXisoExePath);

        using var wrapper = exeAvailable ? new ExtractXisoWrapper(extractXisoExePath) : null;

        if (wrapper != null)
        {
            _extractXisoVersion = wrapper.GetVersion();
        }
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "ExtractXisoTester",
