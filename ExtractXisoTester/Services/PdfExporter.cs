...
public static class PdfExporter
{
    ...
    public static void Export(TestSessionResult session, string? extractXisoVersion, string outputPath)
    {
        Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                {
                    header.Item().Text("ExtractXiso Tester — Results Report")
                        .Bold().FontSize(16).FontColor(Colors.Blue.Darken3);

                    var genText = $"Generated: {DateTime.Now:yyyy-MM-dd HH:mm:ss}";
                    if (extractXisoVersion != null)
                    {
                        genText += $"    extract-xiso: {extractXisoVersion}";
                    }
                ...
                page.Footer().AlignCenter()
                    .Text("ExtractXiso Tester — generated with QuestPDF").FontSize(7)
