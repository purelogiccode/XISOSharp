...
internal static class ZstdBlockEncoder
{
    ...
    public static int EncodeBlock(
        ...
        EncodeBlock(src, level, dst, dstOffset, dstCapacity, lastBlock, ZstdSeq.FreshRepeatOffsets());
        ...
        byte[] dst, int dstOffset, int dstCapacity, bool lastBlock, uint[] rep)
    {
        ...
        uint header = (lastBlock ? 1u : 0u) | ((uint)BlockTypeCompressed << 1) | ((uint)payload << 3);
    ...
    {
        int start = pos;
        int litLen = store.LiteralLength + store.TrailingLength;
        if (litLen == 0)
        {
        ...
        if (AllIdentical(litBuf, out byte repeated))
        {
        ...
        int lhSize = 3 + (litLen >= 1024 ? 1 : 0) + (litLen >= 16384 ? 1 : 0);
        int huffSize = 0;
        if (end - (pos + lhSize) > 0)
        {
        ...
        if (huffSize >= 2)
        {
            bool singleStream = litLen < ZstdHuffmanEncoder.SingleStreamThreshold;
            int sizeFormat = lhSize == 3 ? (singleStream ? 0 : 1) : lhSize == 4 ? 2 : 3;
            int regenBits = sizeFormat <= 1 ? 10 : sizeFormat == 2 ? 14 : 18;
    ...
    {
        int start = pos;
        int flSize = 1 + (litLen > 31 ? 1 : 0) + (litLen > 4095 ? 1 : 0);
        int extra = mode == SetRle ? 1 : litLen;
        ...
        {
            uint header = mode + (1u << 2) + ((uint)litLen << 4);
            dst[pos] = (byte)header;
            dst[pos + 1] = (byte)(header >> 8);
        }
        else
        {
            uint header = mode + (3u << 2) + ((uint)litLen << 4);
            dst[pos] = (byte)header;
            dst[pos + 1] = (byte)(header >> 8);
            dst[pos + 2] = (byte)(header >> 16);
            dst[pos + 3] = (byte)(header >> 24);
        ...
        ZstdSequenceStore store, int nbSeq, byte[] dst, int pos, int end)
    {
        ...
        for (int i = 0; i < nbSeq; i++)
        {
            ...
            int llCode = LLcode(seq.LitLength);
            uint mlBase = seq.MatchLength - (uint)ZstdSeq.MinMatch;
            int mlCode = MLcode(mlBase);
            int ofCode = BitOperations.Log2(seq.OffBase);
            if (ofCode >= 32)
            {
        ...
        SeqAlphabet ll = BuildSeqTable(llCount, llCodes, nbSeq, llMax, LLFseLog);
        SeqAlphabet of = BuildSeqTable(ofCount, ofCodes, nbSeq, ofMax, OffFseLog);
        SeqAlphabet ml = BuildSeqTable(mlCount, mlCodes, nbSeq, mlMax, MLFseLog);
        ...
        FseCTable llTable = ll.Table;
        FseCTable ofTable = of.Table;
        FseCTable mlTable = ml.Table;
        ...
        int last = nbSeq - 1;
        long stateML = ZstdFseEncoder.InitCState2(mlTable, mlCodes[last]);
        long stateOF = ZstdFseEncoder.InitCState2(ofTable, ofCodes[last]);
        long stateLL = ZstdFseEncoder.InitCState2(llTable, llCodes[last]);
        ...
        uint[] count, byte[] codes, int nbSeq, int maxObserved, int maxLog)
    {
        ...
        int tableLog = ZstdFseEncoder.OptimalTableLog(maxLog, nbSeq, maxObserved);
        int total = nbSeq;
        byte lastCode = codes[nbSeq - 1];
        if (count[lastCode] > 1)
        {
            count[lastCode]--;
            total--;
        ...
        bool useLowProb = total >= 2048; // ZSTD_useLowProbCount.
        int got = ZstdFseEncoder.NormalizeCounts(norm, count, total, maxObserved, tableLog, useLowProb);
        if (got == -1)
        {
            throw new ZstdException("Sequence alphabet unexpectedly RLE.");
        }

        FseCTable table = ZstdFseEncoder.BuildCTable(norm, maxObserved, tableLog);
        ...
        byte[] dst, int pos, int end, SeqAlphabet alphabet, byte rleSymbol)
    {
        ...
        int size = ZstdFseEncoder.WriteNCount(
    ...
    private static int LLcode(uint litLength) =>
        litLength > 63 ? BitOperations.Log2(litLength) + 19 : LLCodeTable[litLength];

    private static int MLcode(uint mlBase) =>
        mlBase > 127 ? BitOperations.Log2(mlBase) + 36 : MLCodeTable[mlBase];
